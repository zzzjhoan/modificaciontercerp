...
btnEvaluar.onclick=()=>{
  ...
  if(!r1.checked && !r2.checked && !r3.checked && !r4.checked  ){
    ...
    if(aciertos>=3){
      Swal.fire({  icon:'success',title:'YES AMIGO', text:'Calificacion'+(aciertos*2), foot:'Quiz' });
    }else{
      Swal.fire({icon:'error',title:'NO LE SABES',text:'Calificacion'+(aciertos*2),footer:'Quiz'});
    }
